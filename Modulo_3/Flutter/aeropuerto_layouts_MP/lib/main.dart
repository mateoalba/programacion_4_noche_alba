...
void main() => runApp(const AppControlVuelos());

class AppControlVuelos extends StatefulWidget {
  const AppControlVuelos({super.key});
  @override
  State<AppControlVuelos> createState() => _AppControlVuelosState();
}

class _AppControlVuelosState extends State<AppControlVuelos> {
...
class _Paso1 extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title:           const Text('Sistema de Control de Vuelos'),
      ...
      body: Center(
        child: Column(
          ...
          children: [
            Icon(Icons.flight, size: 64, color: cs.primary),
            const SizedBox(height: 16),
            Text(
              'Terminal A — Puerta 12',
              style: text.headlineMedium?.copyWith(fontWeight: FontWeight.bold),
            ),
            const SizedBox(height: 8),
            Text(
              'AV1234 · Avianca · 07:00',
